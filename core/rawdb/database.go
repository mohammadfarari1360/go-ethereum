...
func NewDatabaseWithFreezer(db ethdb.KeyValueStore, freezer string, namespace string) (ethdb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			...
			// Otherwise, the key-value store continues where the freezer left off, all is
			// fine. We might have duplicate blocks (crash after freezer write but before
			// key-value store deletion, but that's fine).
		} else {
			...
			// Otherwise, the head header is still the genesis, we're allowed to init a
			// new feezer
		}
	}
